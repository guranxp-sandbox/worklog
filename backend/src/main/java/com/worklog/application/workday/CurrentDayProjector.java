...
public class CurrentDayProjector {
    ...
    public CurrentDayProjector(final CurrentDayProjectionRepository projectionRepository) {
        this.projectionRepository = projectionRepository;
    }

    public void project(final WorkDayId workDayId, final int newVersion, final List<DomainEvent> events) {
        ...
        final CurrentDayView updated = new CurrentDayView(
    ...
    private CurrentDayView apply(final CurrentDayView view, final DomainEvent event) {
        if (event instanceof TimeBlockStarted e) {
            final TimeBlockView active = new TimeBlockView(e.timeBlockId(), e.startedAt(), null, e.projectId(), null);
        ...
        if (event instanceof TimeBlockEnded e) {
            final int minutes = (int) ChronoUnit.MINUTES.between(
                    view.activeTimeBlock().startedAt(), e.endedAt());
            final UUID projectId = e.projectId() != null ? e.projectId()
                    : (view.activeTimeBlock() != null ? view.activeTimeBlock().projectId() : null);
            final TimeBlockView closed = new TimeBlockView(
                    e.timeBlockId(), view.activeTimeBlock().startedAt(), e.endedAt(), projectId, minutes);

            final List<TimeBlockView> completed = new ArrayList<>(view.completedTimeBlocks());
